...
const replacement = `async function executePipelineAsync(runId: string, project: DeploymentProject, isManual: boolean) {
  ...
  run.status = 'running';
  try {
    const result = await runRealPipeline(project, run, updateStage);
    if (!result.success) {
      run.status = 'failed';
    ...
    run.completedAt = new Date().toISOString();
    project.lastDeployStatus = 'failed';
    const index = run.stages.findIndex((stage) => stage.status === 'running');
    if (index >= 0) { run.stages[index].status = 'failed'; run.stages[index].logs.push(message); }
  }
}`
