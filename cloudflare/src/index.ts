...
async function route(request: Request, env: Env) {
  ...
  if (url.pathname === "/api/projects" && request.method === "GET") { const rows = await env.DB.prepare("SELECT * FROM projects ORDER BY updated_at DESC").all(); return json({ projects: rows.results }, 200, origin); }
  if (url.pathname === "/api/projects/upsert" && request.method === "POST") {
    ...
    const now = new Date().toISOString(), existing = await env.DB.prepare("SELECT * FROM projects WHERE repo_url=?").bind(repoUrl).first<any>();
  ...
  if (url.pathname === "/api/pipelines/trigger" && request.method === "POST") {
    const body = await request.json<Record<string, unknown>>(), projectId = String(body.projectId || ""), project = await env.DB.prepare("SELECT * FROM projects WHERE id=?").bind(projectId).first<any>();
    ...
    catch (error) { await env.DB.prepare("UPDATE runs SET status='failed',updated_at=?,message=? WHERE id=?").bind(new Date().toISOString(), String(error), runId).run(); return json({ error: "Unable to dispatch GitHub Actions", run: { id: runId, status: "failed" } }, 502, origin); }
  ...
  if (runMatch && request.method === "GET") { const run = await env.DB.prepare("SELECT * FROM runs WHERE id=?").bind(runMatch[1]).first(); return run ? json({ run }, 200, origin) : json({ error: "Run not found" }, 404, origin); }
